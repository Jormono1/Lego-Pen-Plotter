...
### alt mode: Hub light = Green, Left stick up/down controls Y motor, Right stick left/right controls X motor ###
### Menu (start) button triggers homing sequence, X button Toggles standard/alt modes ###
...
### B button also changes the hub light to Red for visiual feedback ###
### LB + RB combo resets stick speed to initial value ###
...
button_wait = 200                                                               # delay to prevent button bounce on inputs
...
def main(pen_engaged, stick_speed, x_min, y_min, alt_control, button_wait, move_speed, hub):
    while True:
        ...
        X_Motor.run(round(stick_speed * x_can_move(stick_x) * stick_x))         # round because stick speed value can be a float
        ...
        if Button.A in xbox.buttons.pressed():                                  # A button toggles pen engagment
            ...
            Y_Motor.run_target(move_speed, 2700 , wait=True)                    # 2700 was determined arbitrarily to be a good distance back to clear the card
            hold = True
            ...
            if alt_control:
                ...
                alt_control = True
                wait(button_wait)
        elif Button.MENU in xbox.buttons.pressed():                             # menu (start) button triggers homing sequence
            homing(x_min, y_min)
            ...
            if stick_speed < 10:
                stick_speed += 0.5      
                wait(button_wait)     
        elif Button.LB and Button.RB in xbox.buttons.pressed():                 # reset stick speed to starting speed of 5
            stick_speed = 5
            wait(button_wait)     
homing(allow_x_min, allow_y_min)
main(pen_engaged, stick_speed, allow_x_min, allow_y_min, alt_control, button_wait, move_speed, hub)
